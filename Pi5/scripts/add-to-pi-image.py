#!/usr/bin/env python3
"""
Add files to the boot partition of a Raspberry Pi SD card image.

Usage:
    python add-to-pi-image.py <image> <dest_path> <source_file>
    
Examples:
    python add-to-pi-image.py webbos-pi.img apps/calc.html system/apps/calc.html
    python add-to-pi-image.py webbos-pi.img apps/games/invaders.html system/games/invaders.html
"""
...
def parse_fat32_bpb_partitioned(f, partition_start=FIRST_PARTITION_START):
    """Parse FAT32 BPB from a partitioned image."""
...
def cluster_to_offset_partitioned(cluster, bpb):
    """Convert cluster number to byte offset in partitioned image."""
...
def read_cluster_partitioned(f, cluster, bpb):
    """Read a cluster from the partitioned filesystem."""
...
def write_cluster_partitioned(f, cluster, bpb, data):
    """Write data to a cluster in partitioned image."""
    offset = cluster_to_offset_partitioned(cluster, bpb)
...
def read_fat_partitioned(f, bpb):
    """Read FAT table from partitioned image."""
    partition_start = bpb['partition_start']
...
def write_fat_partitioned(f, bpb, fat_data):
    """Write FAT table to all copies in partitioned image."""
    partition_start = bpb['partition_start']
    fat_size = bpb['sectors_per_fat'] * bpb['bytes_per_sector']
...
def add_file_to_pi_image(image_path, dest_path, source_file):
    """Add a file to the Pi SD card image boot partition."""
    with open(image_path, 'r+b') as f:
        ...
        for part in parts[:-1]:
            next_cluster = find_directory_partitioned(f, bpb, current_cluster, part)
            if next_cluster is None:
                print(f"Creating directory: {part}")
                next_cluster = create_directory_partitioned(f, bpb, current_cluster, part)
        ...
        add_directory_entry_partitioned(f, bpb, current_cluster, name, ext, 0x20, clusters[0], file_size)
...
def main():
    if len(sys.argv) < 4:
        ...
        print(f"Example: {sys.argv[0]} webbos-pi.img apps/calc.html system/apps/calc.html")
        sys.exit(1)
    ...
    add_file_to_pi_image(image_path, dest_path, source_file)
