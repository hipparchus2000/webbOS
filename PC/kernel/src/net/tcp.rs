...
fn generate_isn(id: &ConnectionId) -> u32 {
    ...
    // Read x86 TSC (Time Stamp Counter) - provides high-resolution timing
    ...
    hash_input[0..8].copy_from_slice(&tsc.to_le_bytes());
    ...
    if secret == 0 {
        ...
        let new_secret = tsc.wrapping_mul(0x9E3779B97F4A7C15); // Golden ratio constant
    ...
    // Assuming 2.5GHz CPU, TSC ticks per microsecond = 2500
    let micros = tsc / 2500; // Approximate conversion assuming 2.5GHz CPU
