...
impl DhcpClient {
    ...
    /// Get configuration tuple (ip, subnet_mask, gateway)
    pub fn get_config(&self) -> Option<(Ipv4Address, Ipv4Address, Ipv4Address)> {
        Some((self.assigned_ip?, self.subnet_mask?, self.gateway?))
    }
    
    /// Create DHCP REQUEST message and return bytes
    pub fn send_request(&mut self) -> Vec<u8> {
        self.state = DhcpState::Requesting;
        let server_id = match self.server_id {
            Some(ip) => ip,
            None => Ipv4Address::new([0, 0, 0, 0]),
        };
        let requested_ip = match self.assigned_ip {
            Some(ip) => ip,
            None => Ipv4Address::new([0, 0, 0, 0]),
        };
        self.create_request(server_id, requested_ip)
    }
    
    /// Handle DHCP ACK message
    pub fn handle_ack(&mut self, msg: &DhcpMessage) -> Option<()> {
        self.assigned_ip = Some(msg.yiaddr);
        self.state = DhcpState::Bound;
        self.lease_start = elapsed_ms();
        self.last_activity = elapsed_ms();
        
        // Parse options for subnet_mask, gateway, lease_time
        let mut i = 0;
        while i < msg.options.len() {
            if i + 1 >= msg.options.len() {
                break;
            }
            let code = msg.options[i];
            let len = msg.options[i + 1] as usize;
            
            if i + 2 + len > msg.options.len() {
                break;
            }
            ...
            match code {
                1 => { // Subnet mask
                    if len == 4 {
                        self.subnet_mask = Some(Ipv4Address::new([
                            msg.options[i + 2], msg.options[i + 3],
                            msg.options[i + 4], msg.options[i + 5]
                        ]));
                    }
                }
                3 => { // Router/gateway
                    if len >= 4 {
                        self.gateway = Some(Ipv4Address::new([
                            msg.options[i + 2], msg.options[i + 3],
                            msg.options[i + 4], msg.options[i + 5]
                        ]));
                    }
                ...
                6 => { // DNS servers
                    if len >= 4 {
                        self.dns_servers.push(Ipv4Address::new([
                            msg.options[i + 2], msg.options[i + 3],
                            msg.options[i + 4], msg.options[i + 5]
                        ]));
                    }
                ...
                51 => { // Lease time
                    if len == 4 {
                        self.lease_time = ((msg.options[i + 2] as u32) << 24) |
                                         ((msg.options[i + 3] as u32) << 16) |
                                         ((msg.options[i + 4] as u32) << 8) |
                                         (msg.options[i + 5] as u32);
                    }
                }
                54 => { // Server ID
                    if len == 4 {
                        self.server_id = Some(Ipv4Address::new([
                            msg.options[i + 2], msg.options[i + 3],
                            msg.options[i + 4], msg.options[i + 5]
                        ]));
                    }
    ...
    /// Create DHCP renewal message
    pub fn send_renewal(&mut self) -> Vec<u8> {
...
/// Start DHCP discovery (convenience function for backward compatibility)
/// 
/// Uses a default MAC address. For production use, create a DhcpClient with
/// the actual MAC address of your network interface.
pub fn start_dhcp() {
    use crate::net::MacAddress;
    
    println!("[dhcp] Starting DHCP discovery...");
    
    // Default MAC address (should be replaced with actual interface MAC)
    let mac = MacAddress::new([0x52, 0x54, 0x00, 0x12, 0x34, 0x56]);
    let mut client = DhcpClient::new(mac);
    
    // Start discovery
    let _ = client.start_discovery();
    
    println!("[dhcp] DHCP discovery started (using default MAC)");
}
