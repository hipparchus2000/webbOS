...
/// Initialize ATA drives (x86_64 only)
pub fn init() {
...
/// ATA drive stub for non-x86_64 platforms
#[cfg(not(target_arch = "x86_64"))]
pub struct AtaDrive;
...
impl AtaDrive {
    /// Create stub
    pub fn new(_base_port: u16, _control_port: u16, _is_master: bool) -> Self {
        Self
    }
}
...
impl BlockDevice for AtaDrive {
    ...
    fn read_blocks(&self, _start: u64, _count: usize, _buf: &mut [u8]) -> Result<(), StorageError> {
...
/// Initialize ATA drives (stub on non-x86_64)
#[cfg(not(target_arch = "x86_64"))]
pub fn init() {
    // ATA not supported on ARM
}
